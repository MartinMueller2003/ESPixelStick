...
#if _SOC_I2S_SUPPORTS_DAC
#include "driver/dac.h"
#endif // _SOC_I2S_SUPPORTS_DAC

#if _SOC_I2S_SUPPORTS_ADC
#include "adc1_private.h"
#endif // _SOC_I2S_SUPPORTS_ADC
 ...
 */
void m_gpio_matrix_out_check_and_set(gpio_num_t gpio, uint32_t signal_idx, bool out_inv, bool oen_inv)
{
    //if pin = -1, disconnect the signal from GPIO matrix, and connect the signal to SIG_GPIO_OUT_IDX, which is a dummy output signal.
    if (gpio == -1) 
    {
        esp_rom_gpio_connect_out_signal(signal_idx, SIG_GPIO_OUT_IDX, false, false);
    }
    else
    {
 ...
 * @param   oen_inv       Output eanble control invert enable
 */
static void m_gpio_matrix_in_check_and_set(gpio_num_t gpio, uint32_t signal_idx, bool inv)
{
    if (gpio != -1) {
        gpio_hal_iomux_func_sel(GPIO_PIN_MUX_REG[gpio], PIN_FUNC_GPIO);
 ...
 */
static esp_err_t m_i2s_check_set_mclk(i2s_port_t i2s_num, gpio_num_t gpio_num)
{
    if (gpio_num == -1) {
        return ESP_OK;
    }
 ...
 *      - ESP_ERR_INVALID_ARG   GPIO is not available
 */
esp_err_t m_i2s_set_pin(i2s_port_t i2s_num, const i2s_pin_config_t *pin)
{
    ESP_RETURN_ON_FALSE((i2s_num < I2S_NUM_MAX), ESP_ERR_INVALID_ARG, TAG, "i2s_num error");
    if (pin == NULL) {
#if _SOC_I2S_SUPPORTS_DAC
        return m_i2s_set_dac_mode(I2S_DAC_CHANNEL_BOTH_EN);
#else
    ...
    if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_SLAVE) {
        ...
        if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_RX) {
            m_gpio_matrix_in_check_and_set(pin->ws_io_num, i2s_periph_signal[i2s_num].s_rx_ws_sig, 0);
            m_gpio_matrix_in_check_and_set(pin->bck_io_num, i2s_periph_signal[i2s_num].s_rx_bck_sig, 0);
            /* For "tx + slave" mode, we should select TX signal index for ws and bck */
        } else {
            m_gpio_matrix_in_check_and_set(pin->ws_io_num, i2s_periph_signal[i2s_num].s_tx_ws_sig, 0);
            m_gpio_matrix_in_check_and_set(pin->bck_io_num, i2s_periph_signal[i2s_num].s_tx_bck_sig, 0);
        }
    } else {
        /* mclk only available in master mode */
        ESP_RETURN_ON_ERROR(m_i2s_check_set_mclk(i2s_num, pin->mck_io_num), TAG, "mclk config failed");
        ...
        if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_TX) {
            m_gpio_matrix_out_check_and_set(pin->ws_io_num, i2s_periph_signal[i2s_num].m_tx_ws_sig, 0, 0);
            m_gpio_matrix_out_check_and_set(pin->bck_io_num, i2s_periph_signal[i2s_num].m_tx_bck_sig, 0, 0);
            /* For "rx + master" mode, we should select RX signal index for ws and bck */
        } else {
            m_gpio_matrix_out_check_and_set(pin->ws_io_num, i2s_periph_signal[i2s_num].m_rx_ws_sig, 0, 0);
            m_gpio_matrix_out_check_and_set(pin->bck_io_num, i2s_periph_signal[i2s_num].m_rx_bck_sig, 0, 0);
        }
    }

    /* Set data input/ouput GPIO */
    m_gpio_matrix_out_check_and_set(pin->data_out_num, i2s_periph_signal[i2s_num].data_out_sig, 0, 0);
    m_gpio_matrix_in_check_and_set(pin->data_in_num, i2s_periph_signal[i2s_num].data_in_sig, 0);
    return ESP_OK;
}
 ...
 *      - false no need
 */
static bool IRAM_ATTR m_i2s_dma_rx_callback(gdma_channel_handle_t dma_chan, gdma_event_data_t *event_data, void *user_data)
{
 ...
 *      - whether need yield
 */
static bool IRAM_ATTR m_i2s_dma_tx_callback(gdma_channel_handle_t dma_chan, gdma_event_data_t *event_data, void *user_data)
{
    i2s_obj_t *p_i2s = (i2s_obj_t *) user_data;
    portBASE_TYPE need_awoke = 0;
    portBASE_TYPE tmp = 0;
 ...
 */
static void IRAM_ATTR m_i2s_intr_handler_default(void *arg)
{
 ...
 */
static esp_err_t m_i2s_dma_intr_init(i2s_port_t i2s_num, int intr_flag)
{
#if SOC_GDMA_SUPPORTED
    ...
    if ( p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_TX) {
        ...
        gdma_tx_event_callbacks_t cb = {.on_trans_eof = m_i2s_dma_tx_callback};
    ...
    if ( p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_RX) {
        ...
        gdma_rx_event_callbacks_t cb = {.on_recv_eof = m_i2s_dma_rx_callback};
        /* Set callback function for GDMA, the interrupt is triggered by GDMA, then the GDMA ISR will call the  callback function */
        gdma_register_rx_event_callbacks(p_i2s[i2s_num]->rx_dma_chan, &cb, p_i2s[i2s_num]);
    }
#else
    /* Initial I2S module interrupt */
    ESP_RETURN_ON_ERROR(esp_intr_alloc(i2s_periph_signal[i2s_num].irq, intr_flag, m_i2s_intr_handler_default, p_i2s[i2s_num], &p_i2s[i2s_num]->i2s_isr_handle), TAG, "Register I2S Interrupt error");
 ...
 * @param   i2s_num     I2S device number
 */
static void m_i2s_tx_reset(i2s_port_t i2s_num)
{
 ...
 * @param   i2s_num     I2S device number
 */
static void m_i2s_rx_reset(i2s_port_t i2s_num)
{
 ...
 * @param   i2s_num     I2S device number
 */
static void m_i2s_tx_start(i2s_port_t i2s_num)
{
#if SOC_GDMA_SUPPORTED
 ...
 * @param   i2s_num     I2S device number
 */
static void m_i2s_tx_stop(i2s_port_t i2s_num)
{
 ...
 * @param   i2s_num     I2S device number
 */
static void m_i2s_rx_stop(i2s_port_t i2s_num)
{
 ...
 *      - DMA buffer size
 */
static inline uint32_t m_i2s_get_buf_size(i2s_port_t i2s_num)
{
 ...
 */
static esp_err_t m_i2s_delete_dma_buffer(i2s_port_t i2s_num, i2s_dma_t *dma_obj)
{
 ...
 */
static esp_err_t m_i2s_alloc_dma_buffer(i2s_port_t i2s_num, i2s_dma_t *dma_obj)
{
...
err:
    ...
    m_i2s_delete_dma_buffer(i2s_num, dma_obj);
    return ret;
}
 ...
 */
static esp_err_t m_i2s_realloc_dma_buffer(i2s_port_t i2s_num, i2s_dma_t *dma_obj)
{
    ESP_RETURN_ON_FALSE(dma_obj, ESP_ERR_INVALID_ARG, TAG, "I2S DMA object can't be NULL");
    ...
    ESP_RETURN_ON_ERROR(m_i2s_alloc_dma_buffer(i2s_num, dma_obj), TAG, "Failed to allocate dma buffer");
 ...
 */
static esp_err_t m_i2s_destroy_dma_object(i2s_port_t i2s_num, i2s_dma_t **dma)
{
    ...
    m_i2s_delete_dma_buffer(i2s_num, (*dma));
 ...
 */
static esp_err_t m_i2s_create_dma_object(i2s_port_t i2s_num, i2s_dma_t **dma)
{
...
err:
    ...
    m_i2s_destroy_dma_object(i2s_num, dma);
 ...
 */
esp_err_t m_i2s_zero_dma_buffer(i2s_port_t i2s_num)
{
    ESP_RETURN_ON_FALSE((i2s_num < I2S_NUM_MAX), ESP_ERR_INVALID_ARG, TAG, "i2s_num error");
    ...
    if (p_i2s[i2s_num]->tx && p_i2s[i2s_num]->tx->buf != NULL && p_i2s[i2s_num]->tx->buf_size != 0) {
        ...
        if (bytes_left) {
            ...
            m_i2s_write(i2s_num, (void *)&zero_bytes, bytes_left, &bytes_written, portMAX_DELAY);
        }
        for (int i = 0; i < p_i2s[i2s_num]->dma_buf_count; i++) {
...
static esp_err_t m_i2s_set_apll_freq(uint32_t expt_freq, uint32_t *real_freq)
{
 ...
 */
static uint32_t m_i2s_config_source_clock(i2s_port_t i2s_num, bool use_apll, uint32_t mclk)
{
#if SOC_I2S_SUPPORTS_APLL
    if (use_apll) {
        ...
        esp_err_t ret = m_i2s_set_apll_freq(expt_freq, &real_freq);
        if (ret != ESP_OK) {
...
#if _SOC_I2S_SUPPORTS_ADC || _SOC_I2S_SUPPORTS_DAC
/**
 ...
 */
static esp_err_t m_i2s_calculate_adc_dac_clock(int i2s_num, i2s_hal_clock_cfg_t *clk_cfg)
{
    ...
    clk_cfg->sclk = m_i2s_config_source_clock(i2s_num, p_i2s[i2s_num]->use_apll, clk_cfg->mclk);
...
#endif // _SOC_I2S_SUPPORTS_ADC || _SOC_I2S_SUPPORTS_DAC
 ...
 *      - ESP_ERR_INVALID_ARG   Invalid args
 */
static esp_err_t m_i2s_calculate_pdm_tx_clock(int i2s_num, i2s_hal_clock_cfg_t *clk_cfg)
{
    ESP_RETURN_ON_FALSE(clk_cfg, ESP_ERR_INVALID_ARG, TAG, "input clk_cfg is NULL");
 ...
 *      - ESP_ERR_INVALID_ARG   Invalid args
 */
static esp_err_t m_i2s_calculate_pdm_rx_clock(int i2s_num, i2s_hal_clock_cfg_t *clk_cfg)
{
    ESP_RETURN_ON_FALSE(clk_cfg, ESP_ERR_INVALID_ARG, TAG, "input clk_cfg is NULL");
    ESP_RETURN_ON_FALSE(p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_PDM, ESP_ERR_INVALID_ARG, TAG, "current mode is not PDM");
 ...
 *      - ESP_ERR_INVALID_ARG   Invalid args
 */
static esp_err_t m_i2s_calculate_common_clock(int i2s_num, i2s_hal_clock_cfg_t *clk_cfg)
{
    ESP_RETURN_ON_FALSE(clk_cfg, ESP_ERR_INVALID_ARG, TAG, "input clk_cfg is NULL");
 ...
 *      - ESP_ERR_INVALID_ARG   Invalid args
 */
static esp_err_t m_i2s_calculate_clock(i2s_port_t i2s_num, i2s_hal_clock_cfg_t *clk_cfg)
{
    /* Calculate clock for ADC mode */
#if _SOC_I2S_SUPPORTS_ADC
    if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_ADC_BUILT_IN) {
        ESP_RETURN_ON_ERROR(m_i2s_calculate_adc_dac_clock(i2s_num, clk_cfg), TAG, "ADC clock calculate failed");
        return ESP_OK;
    }
#endif // _SOC_I2S_SUPPORTS_ADC
    ...
    if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_DAC_BUILT_IN) {
        ESP_RETURN_ON_ERROR(m_i2s_calculate_adc_dac_clock(i2s_num, clk_cfg), TAG, "DAC clock calculate failed");
        return ESP_OK;
    }
#endif // _SOC_I2S_SUPPORTS_DAC
...
#if SOC_I2S_SUPPORTS_PDM_TX
        if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_TX) {
            ESP_RETURN_ON_ERROR(m_i2s_calculate_pdm_tx_clock(i2s_num, clk_cfg), TAG, "PDM TX clock calculate failed");
        }
#endif // SOC_I2S_SUPPORTS_PDM_TX
#if SOC_I2S_SUPPORTS_PDM_RX
        if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_RX) {
            ESP_RETURN_ON_ERROR(m_i2s_calculate_pdm_rx_clock(i2s_num, clk_cfg), TAG, "PDM RX clock calculate failed");
        }
#endif // SOC_I2S_SUPPORTS_PDM_RX
        return ESP_OK;
    }
    ...
    ESP_RETURN_ON_ERROR(m_i2s_calculate_common_clock(i2s_num, clk_cfg), TAG, "Common clock calculate failed");
    return ESP_OK;
}
 ...
 */
static uint32_t m_i2s_get_max_channel_num(i2s_channel_t chan_mask)
{
    uint32_t max_chan = 0;
 ...
 */
static uint32_t m_i2s_get_active_channel_num(const i2s_hal_config_t *hal_cfg)
{
 ...
 */
esp_err_t m_i2s_set_dac_mode(i2s_dac_mode_t dac_mode)
{
 ...
 */
static esp_err_t m__i2s_adc_mode_recover(void)
{
 ...
 *      - ESP_ERR_INVALID_ARG   ADC not initialized yet
 */
esp_err_t m_i2s_set_adc_mode(adc_unit_t adc_unit, adc1_channel_t adc_channel)
{
 ...
 */
esp_err_t m_i2s_adc_enable(i2s_port_t i2s_num)
{
    ESP_RETURN_ON_FALSE((i2s_num < I2S_NUM_MAX), ESP_ERR_INVALID_ARG, TAG, "i2s_num error");
    ...
    m__i2s_adc_mode_recover();
    m_i2s_rx_reset(i2s_num);
    return m_i2s_set_clk(i2s_num, p_i2s[i2s_num]->hal_cfg.sample_rate, p_i2s[i2s_num]->hal_cfg.sample_bits, p_i2s[i2s_num]->hal_cfg.active_chan);
}
 ...
 *      - ESP_ERR_INVALID_STATE Current I2S mode is not built-in ADC
 */
esp_err_t m_i2s_adc_disable(i2s_port_t i2s_num)
{
    ESP_RETURN_ON_FALSE((i2s_num < I2S_NUM_MAX), ESP_ERR_INVALID_ARG, TAG, "i2s_num error");
    ESP_RETURN_ON_FALSE((p_i2s[i2s_num] != NULL), ESP_ERR_INVALID_STATE, TAG, "Not initialized yet");
    ESP_RETURN_ON_FALSE((p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_ADC_BUILT_IN), ESP_ERR_INVALID_STATE, TAG, "i2s built-in adc not enabled");
 ...
 */
esp_err_t m_i2s_set_sample_rates(i2s_port_t i2s_num, uint32_t rate)
{
    ESP_RETURN_ON_FALSE((i2s_num < I2S_NUM_MAX), ESP_ERR_INVALID_ARG, TAG, "i2s_num error");
    ESP_RETURN_ON_FALSE((p_i2s[i2s_num]->hal_cfg.sample_bits > 0), ESP_ERR_INVALID_ARG, TAG, "sample bits not set");
    return m_i2s_set_clk(i2s_num, rate, p_i2s[i2s_num]->hal_cfg.sample_bits, p_i2s[i2s_num]->hal_cfg.active_chan);
}
 ...
 *     - ESP_ERR_INVALID_ARG Parameter error
 */
esp_err_t m_i2s_pcm_config(i2s_port_t i2s_num, const i2s_pcm_cfg_t *pcm_cfg)
{
 ...
 *     - ESP_ERR_NO_MEM      Out of memory
 */
esp_err_t m_i2s_set_pdm_rx_down_sample(i2s_port_t i2s_num, i2s_pdm_dsr_t downsample)
{
    ESP_RETURN_ON_FALSE(p_i2s[i2s_num], ESP_FAIL, TAG, "i2s has not installed yet");
 ...
 *     - ESP_ERR_NO_MEM      Out of memory
 */
esp_err_t m_i2s_set_pdm_tx_up_sample(i2s_port_t i2s_num, const i2s_pdm_tx_upsample_cfg_t *upsample_cfg)
{
    ESP_RETURN_ON_FALSE(p_i2s[i2s_num], ESP_FAIL, TAG, "i2s has not installed yet");
    ESP_RETURN_ON_FALSE((p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_PDM), ESP_ERR_INVALID_ARG, TAG, "i2s mode is not PDM mode");
    ...
    m_i2s_start(i2s_num);
    xSemaphoreGive(p_i2s[i2s_num]->rx->mux);
    return m_i2s_set_clk(i2s_num, upsample_cfg->sample_rate, p_i2s[i2s_num]->hal_cfg.sample_bits, p_i2s[i2s_num]->hal_cfg.active_chan);
}
#endif
 ...
 */
static esp_err_t m_i2s_check_cfg_validity(i2s_port_t i2s_num, i2s_hal_config_t *cfg)
{
#if SOC_I2S_SUPPORTS_PDM_TX || SOC_I2S_SUPPORTS_PDM_RX
    /* Check PDM mode */
...
static void m_i2s_tx_set_clk_and_channel(i2s_port_t i2s_num, i2s_hal_clock_cfg_t *clk_cfg)
{
...
static void m_i2s_rx_set_clk_and_channel(i2s_port_t i2s_num, i2s_hal_clock_cfg_t *clk_cfg)
{
 ...
 *      - sample rate
 */
float m_i2s_get_clk(i2s_port_t i2s_num)
{
    ESP_RETURN_ON_FALSE((i2s_num < I2S_NUM_MAX), ESP_ERR_INVALID_ARG, TAG, "i2s_num error");
 ...
 *     - ESP_ERR_NO_MEM      Out of memory
 */
esp_err_t m_i2s_set_clk(i2s_port_t i2s_num, uint32_t rate, uint32_t bits_cfg, i2s_channel_t ch)
{
    esp_err_t ret = ESP_OK;
    ...
    if (p_i2s[i2s_num]->last_buf_size) {
        ...
        if (ch & I2S_CHANNEL_MONO) {
            if (ch >> 16) {
                ...
                cfg->total_chan = m_i2s_get_max_channel_num(ch);
            } else {
            ...
            if (ch >> 16) {
                ...
                cfg->total_chan = m_i2s_get_max_channel_num(cfg->chan_mask);
            } else {
        ...
        cfg->active_chan = m_i2s_get_active_channel_num(cfg);
        cfg->total_chan = 2;
#endif
        if (cfg->mode & I2S_MODE_TX) {
    ...
    m_i2s_calculate_clock(i2s_num, &clk_cfg);

    uint32_t buf_size = m_i2s_get_buf_size(i2s_num);
    ...
    if (cfg->mode & I2S_MODE_TX) {
        ...
        m_i2s_tx_set_clk_and_channel(i2s_num, &clk_cfg);
        ...
        if (need_realloc) {
            ...
            ret = m_i2s_realloc_dma_buffer(i2s_num, p_i2s[i2s_num]->tx);
        }
    ...
    if (cfg->mode & I2S_MODE_RX) {
        ...
        m_i2s_rx_set_clk_and_channel(i2s_num, &clk_cfg);
        /* If buffer size changed, the DMA buffer need realloc */
        if (need_realloc) {
            p_i2s[i2s_num]->rx->buf_size = buf_size;
            ret = m_i2s_realloc_dma_buffer(i2s_num, p_i2s[i2s_num]->rx);
 ...
 *     - ESP_ERR_INVALID_ARG Parameter error
 */
esp_err_t m_i2s_start(i2s_port_t i2s_num)
{
    ESP_RETURN_ON_FALSE((i2s_num < I2S_NUM_MAX), ESP_ERR_INVALID_ARG, TAG, "i2s_num error");
    //start DMA link
    I2S_ENTER_CRITICAL(i2s_num);
    ...
    if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_TX) {
        m_i2s_tx_reset(i2s_num);
        m_i2s_tx_start(i2s_num);
    }
    if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_RX) {
        m_i2s_rx_reset(i2s_num);
 ...
 *     - ESP_ERR_INVALID_ARG Parameter error
 */
esp_err_t m_i2s_stop(i2s_port_t i2s_num)
{
    ESP_RETURN_ON_FALSE((i2s_num < I2S_NUM_MAX), ESP_ERR_INVALID_ARG, TAG, "i2s_num error");
    I2S_ENTER_CRITICAL(i2s_num);
#if !SOC_GDMA_SUPPORTED
...
#endif
    if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_TX) {
        m_i2s_tx_stop(i2s_num);
    }
    if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_RX) {
        m_i2s_rx_stop(i2s_num);
    }
#if !SOC_GDMA_SUPPORTED
 ...
 */
static esp_err_t m_i2s_driver_init(i2s_port_t i2s_num, const i2s_config_t *i2s_config)
{
    ...
    case I2S_CHANNEL_FMT_MULTIPLE:
        ...
        uint32_t max_channel = m_i2s_get_max_channel_num(p_i2s[i2s_num]->hal_cfg.chan_mask);
...
#else
    /* Calculate actived channel number in channel mask */
    p_i2s[i2s_num]->hal_cfg.active_chan   = m_i2s_get_active_channel_num(&p_i2s[i2s_num]->hal_cfg);
 ...
 */
static esp_err_t m_i2s_dma_object_init(i2s_port_t i2s_num)
{
    uint32_t buf_size = m_i2s_get_buf_size(i2s_num);
    /* Create DMA object */
    if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_TX) {
        ESP_RETURN_ON_ERROR(m_i2s_create_dma_object(i2s_num, &p_i2s[i2s_num]->tx), TAG, "I2S TX DMA object create failed");
        p_i2s[i2s_num]->tx->buf_size = buf_size;
    }
    if (p_i2s[i2s_num]->hal_cfg.mode & I2S_MODE_RX) {
        ESP_RETURN_ON_ERROR(m_i2s_create_dma_object(i2s_num, &p_i2s[i2s_num]->rx), TAG, "I2S RX DMA object create failed");
        p_i2s[i2s_num]->rx->buf_size = buf_size;
    }
    return ESP_OK;
}
 ...
 */
esp_err_t m_i2s_driver_install(i2s_port_t i2s_num, const i2s_config_t *i2s_config, int queue_size, void *i2s_queue)
{
    esp_err_t ret = ESP_OK;
    ...
    ret = m_i2s_priv_register_object(pre_alloc_i2s_obj, i2s_num);
    if (ret != ESP_OK) {
    ...
    ESP_GOTO_ON_ERROR(m_i2s_driver_init(i2s_num, i2s_config), err, TAG, "I2S init failed");
    /* Check the validity of I2S configuration */
    ESP_GOTO_ON_ERROR(m_i2s_check_cfg_validity(i2s_num, &(pre_alloc_i2s_obj->hal_cfg)), err, TAG, "I2S configuration is invalid");
    ...
    ESP_GOTO_ON_ERROR(m_i2s_dma_intr_init(i2s_num, i2s_config->intr_alloc_flags), err, TAG, "I2S interrupt initailze failed");
    /* Initialize I2S DMA object */
    ESP_GOTO_ON_ERROR(m_i2s_dma_object_init(i2s_num), err, TAG,  "I2S dma object create failed");
    ...
    ESP_GOTO_ON_ERROR(m_i2s_set_clk(i2s_num, 0, 0, 0), err, TAG, "I2S set clock failed");

    return ESP_OK;

err:
    /* I2S install failed, prepare to uninstall */
    m_i2s_driver_uninstall(i2s_num);
    return ret;
}
 ...
 *     - ESP_ERR_INVALID_ARG Parameter error
 */
esp_err_t m_i2s_driver_uninstall(i2s_port_t i2s_num)
{
    ESP_RETURN_ON_FALSE(i2s_num < I2S_NUM_MAX, ESP_ERR_INVALID_ARG, TAG, "i2s_num error");
    ...
    m_i2s_stop(i2s_num);
#if _SOC_I2S_SUPPORTS_DAC
    m_i2s_set_dac_mode(I2S_DAC_CHANNEL_DISABLE);
#endif
#if SOC_GDMA_SUPPORTED
    ...
    m_i2s_destroy_dma_object(i2s_num, &p_i2s[i2s_num]->tx);
    m_i2s_destroy_dma_object(i2s_num, &p_i2s[i2s_num]->rx);
    ...
    m_i2s_priv_deregister_object(i2s_num);
    free(obj);
    return ESP_OK;
}
 ...
 *     - ESP_ERR_INVALID_ARG  Parameter error
 */
esp_err_t m_i2s_write(i2s_port_t i2s_num, const void *src, size_t size, size_t *bytes_written, TickType_t ticks_to_wait)
{
 ...
 *     - ESP_ERR_INVALID_ARG Parameter error
 */
esp_err_t m_i2s_write_expand(i2s_port_t i2s_num, const void *src, size_t size, size_t src_bits, size_t aim_bits, size_t *bytes_written, TickType_t ticks_to_wait)
{
    char *data_ptr;
 ...
 *     - ESP_ERR_INVALID_ARG  Parameter error
 */
esp_err_t m_i2s_read(i2s_port_t i2s_num, void *dest, size_t size, size_t *bytes_read, TickType_t ticks_to_wait)
{
...
esp_err_t m_i2s_priv_register_object(void *driver_obj, int port_id)
{
...
esp_err_t m_i2s_priv_deregister_object(int port_id)
{
